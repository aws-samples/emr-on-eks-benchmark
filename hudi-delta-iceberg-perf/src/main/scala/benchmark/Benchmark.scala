...
abstract class Benchmark(private val conf: BenchmarkConf) {
  ...
  protected def runFunc(
      queryName: String = "",
      iteration: Option[Int] = None,
      ignoreError: Boolean = false)(f: => Unit): Unit = synchronized {
    val iterationStr = iteration.map(i => s" - iteration $i").getOrElse("")
    var banner = s"$queryName$iterationStr"
    log("=" * 80)
    log(s"START: $banner")
  ...
  protected def reportQueryResult(
    name: String,
    durationMs: Long,
    iteration: Option[Int] = None): Unit = {
    queryResults += QueryResult(name, iteration, durationMs = Some(durationMs), errorMsg = None)
  }
  ...
  protected def median(seq: Seq[Double]) : Double = {
    seq.sorted.drop(math.floor(seq.size / 2.0).toInt).head
  }

  protected def median(seq: Seq[Long]) : Long = {
    seq.sorted.drop(math.floor(seq.size / 2.0).toInt).head
  }
}
