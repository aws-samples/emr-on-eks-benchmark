...
class TPCDSDataLoad(conf: TPCDSDataLoadConf) extends Benchmark(conf) {
  ...
  def runInternal(): Unit = {
    ...
    val hudiTableType = "cow" // COPY-ON-WRITE for Hudi
    ...
    require(Seq(1, 30000).contains(conf.scaleInGB), "")
    ...
    tableNamesTpcds.foreach { tableName =>
      ...
      var tableOptions = ""
      if (conf.formatName == "hudi") {
        tableOptions =
          if (!primaryKeys || tablePrimaryKeys(tableName)(0).isEmpty) s"OPTIONS ( type = '$hudiTableType')"
    ...
    tableNamesTpcds.foreach { tableName => {
      if (conf.formatName == "iceberg") {
        runQuery(s"SELECT * FROM ice.${dbName}.$tableName.snapshots",
          printRows = true, queryName = s"file-stats-$tableName")
