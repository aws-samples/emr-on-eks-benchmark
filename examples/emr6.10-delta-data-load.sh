...
--name delta-dataload-glue$benchmarkId \
--execution-role-arn $EMR_ROLE_ARN \
--release-label emr-6.10.0-latest \
--job-driver '{
  "sparkSubmitJobDriver": {
      "entryPoint": "local:///usr/lib/spark/examples/delta/delta-benchmarks.jar",
      "entryPointArguments":["--format","delta","--scale-in-gb","3000","--db-name","emrdelta","--exclude-nulls","True","--benchmark-path","s3://'$S3BUCKET'/emrdelta","--source-path","s3://'$S3BUCKET'/BLOG_TPCDS-TEST-3T-partitioned"],
    ...
    "applicationConfiguration": [
      {
        ...
        "properties": {
          "spark.kubernetes.container.image": "'$ECR_URL'/eks-spark-benchmark:emr6.10-delta-nohive",
