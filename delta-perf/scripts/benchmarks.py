...
class Benchmark:
    ...
    def run(self, cluster_hostname, ssh_id_file, ssh_user):
        ...
        self.install_dependencies_via_ssh()
        self.start_benchmark_via_ssh(jar_path_in_cluster, ssh_user)
...
if [ -x "$(command -v yum)" ]; then
    ...
    echo "Failed to install packages: Package manager not found. You must manually install: $package">&2; exit 1;
        ...
        """.strip()
        self.copy_script_via_ssh(script_file_name, script_file_text)
        print(f">>> Install dependencies script {script_file_name} generated and uploaded\n")

        job_cmd = (f"bash {script_file_name}")
        print(job_cmd)
        run_cmd(job_cmd, stream_output=False)
        print(">>> Dependencies have been installed\n")

    def start_benchmark_via_ssh(self, jar_path, ssh_user):
    ...
    @staticmethod
    def copy_script_via_ssh(script_file_name, script_file_text):
        try:
            ...
            # print(scp_cmd)
            # run_cmd(scp_cmd, stream_output=True)
            run_cmd(f"chmod +x {script_file_name}",throw_on_error=False)
        finally:
    ...
    @staticmethod
    def wait_for_completion(benchmark_id, ssh_user, copy_report=True):
        completed = False
        succeeded = False
        ...
        while not completed:
            ...
            (_, out, _) = run_cmd(f"stat -c '%n:   [%y]   [%s bytes]' {output_file}",throw_on_error=False)
            ...
            (_, out, _) = run_cmd(f"ls {completed_file}",throw_on_error=False)
        ...
        (_, out, _) = run_cmd(f"tail {output_file}", throw_on_error=False)
    ...
    def upload_delta_jars_to_cluster_and_get_version(self, cluster_hostname, ssh_id_file, ssh_user):
        ...
        with WorkingDirectory(delta_repo_dir):
            ...
            run_cmd(
                f"rm -rf {remote_maven_dir}/* .ivy2/cache/io.delta .ivy2/jars/io.delta*", stream_output=True, throw_on_error=False)
            run_cmd(f"mkdir -p {remote_maven_dir}",stream_output=True)
